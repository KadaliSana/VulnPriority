...
:mod:`vulnpriority.web.app`: what the browser may ask for (`AnalyzeRequest`), what a running
...
class WebFinding(WebModel):
    ...
    #: One line of fix guidance from ``vulnpriority.report``, when that package is installed.
...
#
# Everything below is served by ``vulnpriority.web.app``. None of it is written into
...
class WebScanProfile(WebModel):
    ...
    authoritative: bool = False   # True when vulnpriority.scan supplied this description itself
...
class WebAsOf(WebModel):
    ...
    A straight projection of ``vulnpriority.intel.models.AsOfVerdict``. It travels to the
...
class WebScannerEnvironment(WebModel):
    ...
    notice: str = ""
    available: bool = False       # whether vulnpriority.scan could be asked at all
...
class WebNovelty(WebModel):
    """The shape returned when ``vulnpriority.novelty`` is absent.
