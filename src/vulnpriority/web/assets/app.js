 ...
 * Data arrives as window.VULNPRIORITY_DATA from data.js, written by vulnpriority.web.exporter.
 *
 * The page has two lives. Opened from file:// it is exactly what it always was: a static
 * report over the bundled run. Served by vulnpriority.web.app it gains an Analyze tab that can
...
(() => {
  ...
  let D = window.VULNPRIORITY_DATA || {};
  ...
  function renderMeta() {
    ...
    if (!hasData()) {
      ...
      foot.innerHTML = `vulnpriority ${esc(m.package_version || "")} - all processing is local to this machine.`;
      return;
    }
    foot.innerHTML =
      ...
      `vulnpriority ${esc(m.package_version || "")} · generated
  ...
  const POLICY_NAMES = {
    lambdamart: "the vulnpriority ordering",
  ...
  function setMethodVisible(visible, persist = true) {
    ...
    if (persist) {
      try { localStorage.setItem("vulnpriority-method", visible ? "1" : "0"); } catch (_) {}
    }
  }
  ...
  function populateQueueControls() {
    ...
    [...policies].forEach((p) =>
      polSel.appendChild(el(`<option value="${esc(p)}">${esc(p === "rank" ? "vulnpriority ranking" : p)}</option>`)));
    polSel.value = "rank";
  }
  ...
  async function enableInteractive() {
    ...
    try {
      ...
      console.warn("vulnpriority: interactive features unavailable - ", error.message);
      return;
    }
    if (!window.VULNPRIORITY_ANALYZE || typeof window.VULNPRIORITY_ANALYZE.init !== "function") return;
    ...
    window.VULNPRIORITY_ANALYZE.init({
      health,
      session: window.VULNPRIORITY_SESSION || {},
      api: {
  ...
  function init() {
    ...
    let remembered = null;
    try { remembered = localStorage.getItem("vulnpriority-method"); } catch (_) {}
    ...
    document.getElementById("themeToggle").addEventListener("click", () => {
      ...
      try { localStorage.setItem("vulnpriority-theme", next); } catch (_) {}
    });
    try {
      const saved = localStorage.getItem("vulnpriority-theme");
