...
from vulnpriority.core.enums import LLMBackendKind, RankerName
...
def get_ranker(name: RankerName) -> type:
    if name not in RANKERS:
        raise KeyError(f"ranker not registered: {name}. Import vulnpriority.rank to register the built-ins.")
    return RANKERS[name]


def get_backend(kind: LLMBackendKind) -> type:
    if kind not in LLM_BACKENDS:
        raise KeyError(f"LLM backend not registered: {kind}. Import vulnpriority.llm to register the built-ins.")
    return LLM_BACKENDS[kind]


def get_parser(name: str) -> type:
    if name not in SCANNER_PARSERS:
        raise KeyError(f"scanner parser not registered: {name}. Import vulnpriority.ingest to register the built-ins.")
