"""All enumerations used across vulnpriority.
...
class LLMBackendKind(str, Enum):
    ...
    * ``OPENAI_COMPATIBLE`` is *one* backend that reaches many providers, because Groq,
    ...
    :class:`~vulnpriority.llm.guarded.GuardedBackend` in
    :func:`~vulnpriority.llm.factory.build_llm_backend`, so the sandbox, canary, envelope,
    ...
    #: in the order documented by :mod:`vulnpriority.core.resolve`, and settle on ``HEURISTIC``
...
# ---------------------------------------------------------------------------
# Internet exploit intelligence (produced by vulnpriority.intel)
# ---------------------------------------------------------------------------
