...
**Why ``reach_delta`` cannot be negative.** See :mod:`vulnpriority.graph.reachability`:
...
contract and :mod:`vulnpriority.graph.monotone` verifies the property at runtime when
...
from vulnpriority.core.config import ComponentCConfig, PipelineConfig
...
class ReachabilityChainScorer(ChainScorer):
    """Component C implementation of :class:`vulnpriority.core.interfaces.ChainScorer`.
    ...
    def build(self, scan: Scan, enriched: list[EnrichedFinding]) -> AttackGraphSummary:
        ...
        if self.config.verify_monotonicity:
            from vulnpriority.graph.monotone import assert_monotone_under_patching
    ...
    def total_risk_after_patching(self, scan_id: str, patched_finding_ids: set[str]) -> float:
        ...
        and the quantity :mod:`vulnpriority.graph.monotone` asserts is non-increasing in the
        patched set.
        """
        state = self._state(scan_id)
