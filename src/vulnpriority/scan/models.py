...
:class:`vulnpriority.core.models.Scan` - byte-for-byte the same shape a parsed ZAP report
...
from vulnpriority.core.models import Frozen, Scan
...
SCANNER_NAME = "vulnpriority-scan"
...
DEFAULT_USER_AGENT = (
    f"vulnpriority-scan/{__version__} "
...
#: :mod:`vulnpriority.scan.spa` comes out of that file, and a single-page application's
...
class ScanProfile(str, Enum):
    ...
    the three benign probes in :mod:`vulnpriority.scan.active` and nothing else, ever.
    """
...
class ProbeKind(str, Enum):
    ...
    The allowlist that gates these lives in :mod:`vulnpriority.scan.safety`. Adding a member
...
class ScanRequest(Frozen):
    ...
    :func:`vulnpriority.scan.safety.require_authorization` refuses every scan without it.
    """
    ...
    def _honest_agent(cls, value: str) -> str:
        if "vulnpriority" not in (value or "").lower():
            raise ValueError("user_agent must identify this tool (it has to contain 'vulnpriority')")
        return value
...
class CheckFinding(Frozen):
    ...
    :class:`~vulnpriority.core.models.Finding`, so the sandbox sees it for what it is.
    """
    ...
    #: :func:`vulnpriority.scan.checks.collapse_site_wide`. Empty means "compare the evidence",
...
class ScannerEnvironment(Frozen):
    ...
    Returned by :func:`vulnpriority.scan.adapters.scanner_environment` so the web application
...
class ScanOutcome(Frozen):
    ...
    ``scan`` is an ordinary :class:`~vulnpriority.core.models.Scan`. The counters exist so the
