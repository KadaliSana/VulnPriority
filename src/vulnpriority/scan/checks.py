...
A :class:`Check` is a small, pure function over one :class:`~vulnpriority.scan.models.Page`
and a :class:`CheckContext`. It returns :class:`~vulnpriority.scan.models.CheckFinding`
objects, which :mod:`vulnpriority.scan.runner` turns into ordinary
:class:`~vulnpriority.core.models.Finding` records. Checks never touch the network: the
passive ones read a response that has already been fetched, and the active ones read the
result of a probe that :mod:`vulnpriority.scan.active` already sent through the allowlist.
...
from vulnpriority.ingest.normalize import query_parameters, url_path
from vulnpriority.ingest.tech_fingerprint import fingerprint_response
from vulnpriority.scan.models import CheckFinding, FormInfo, Page, ProbeKind, ProbeResult, ScanProfile
from vulnpriority.semantic.cpe_match import parse_version
...
#: Minimum safe versions for the libraries :mod:`vulnpriority.ingest.tech_fingerprint` can
