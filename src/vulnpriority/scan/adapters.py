...
scanner in :mod:`vulnpriority.scan.crawler` is the fallback for a machine with nothing
installed, not the first choice.

This module finds the installed tools, ranks them, maps the requested scan profile onto
each tool's own flags, runs the chosen one, and hands its report to :mod:`vulnpriority.ingest`.
...
from vulnpriority.core.errors import ParseError, VulnPriorityError
...
from vulnpriority.ingest.nikto import NiktoParser          # noqa: F401  (registration side effect)
from vulnpriority.ingest.wapiti import WapitiParser        # noqa: F401  (registration side effect)
...
class ExternalToolError(VulnPriorityError):
...
def _zap_docker_argv(tool: ExternalTool, request: ScanRequest, report: Path) -> list[list[str]]:
    ...
    ``-J`` writes the JSON report into it, which :class:`~vulnpriority.ingest.zap.ZapParser`
    then reads.
...
#:    is also how most people actually have ZAP, and vulnpriority has a first-class ZAP parser.
...
#: :func:`vulnpriority.scan.runner.assess_target`.
    ...
    *,
    ...
    if preferred is not None:
        ...
        notice = (
            ...
            f"({'; '.join(skipped)}). The built-in vulnpriority scanner will run instead. "
            + install_hint_for(profile)
        )
    else:
        notice = (
            "No external scanner was found on PATH, so the built-in vulnpriority scanner will "
...
def describe_external_tools(tools: Sequence[ExternalTool] | None = None) -> str:
    ...
    if not found:
        return (
            ...
            "on PATH. Using the built-in vulnpriority scanner instead. "
    ...
    * ``zap-docker`` in ACTIVE mode runs ``zap-full-scan.py``, where ``-T`` caps startup and
    ...
    if not report.exists() or report.stat().st_size == 0:
        raise ExternalToolError(
            ...
            "Falling back to the built-in vulnpriority scanner is the safe response."
        )
    return report
...
def ingest_report(path: str | Path, app_id: str | None = None) -> Scan:
    """Parse any supported scanner report into a :class:`~vulnpriority.core.models.Scan`.

    Format detection goes through :func:`vulnpriority.ingest.generic.detect_parser` first, so
    ...
    try:
        ...
        if parser is None:
            raise ExternalToolError(
                f"no vulnpriority parser recognises {source}: {first_error}"
            ) from first_error
        try:
...
def parser_for(tool_name: str) -> str | None:
    """Name of the vulnpriority parser that reads a tool's report."""
