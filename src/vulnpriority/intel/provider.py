...
returns :class:`~vulnpriority.intel.models.IntelDocument` objects whose text is untrusted by
...
:mod:`vulnpriority.sandbox.pipeline` and doing it here would hide from the injection detector
what the page actually said -- the same reasoning that keeps
:mod:`vulnpriority.feeds.references` from doing it.
"""
...
from vulnpriority.core.enums import Provenance
...
from vulnpriority.intel.queries import classify_source
...
def build_search_provider(config: IntelConfig) -> BaseSearchProvider:
    ...
    :class:`~vulnpriority.core.models.IntelDocument` objects built by :func:`make_document`, so
    ...
    if config.mode == "offline":
        from vulnpriority.intel.offline import FixtureSearchProvider  # noqa: PLC0415

        return FixtureSearchProvider(config.fixture_path)

    if config.search_provider == "parallel":
        try:
            from vulnpriority.intel.parallel_search import (  # noqa: PLC0415 - kept lazy for symmetry
    ...
    if config.search_provider == "gemini":
        try:
            from vulnpriority.intel.gemini_search import (  # noqa: PLC0415 - optional SDK
    ...
    try:
        from vulnpriority.intel.anthropic_search import (  # noqa: PLC0415 - optional SDK
