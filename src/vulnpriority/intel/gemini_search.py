...
:mod:`vulnpriority.intel` exists to search the internet for exploit material the curated feeds
...
:mod:`vulnpriority.intel.anthropic_search` and DESIGN 6.5.
...
set, and :class:`~vulnpriority.core.models.IntelDocument` rejects anything else outright. There
...
* ``GoogleSearch`` has no ``allowed_domains``. It accepts ``exclude_domains`` only, so
  :attr:`~vulnpriority.core.config.IntelConfig.blocked_domains` maps to the API while
...
from vulnpriority.intel.queries import PHASE1_SYSTEM, classify_source
...
from vulnpriority.llm.openai_compatible import RateLimiter, backoff_delay
...
def build_gemini_client(api_key: str) -> Any:
    ...
    try:
        ...
        raise ConfigError(
            ...
            "install vulnpriority[gemini] or use mode='offline'"
        ) from exc
...
class GeminiSearchProvider(BaseSearchProvider):
    ...
    def __init__(
        ...
        *,
        ...
        :class:`~vulnpriority.intel.anthropic_search.AnthropicSearchProvider`: the intelligence
