...
**The contract types live in :mod:`vulnpriority.core.models`, not here.** ``EnrichedFinding``
...
This module re-exports them so ``vulnpriority.intel`` remains a coherent namespace to import
...
#: step with :class:`~vulnpriority.core.models.ExploitIntelOut`, so drift fails the build
#: rather than a run.
...
class IntelGather(BaseModel):
    ...
    downstream consumes. What survives into :class:`~vulnpriority.core.models.IntelResult` is
