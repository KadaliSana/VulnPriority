"""The two-phase exploit-intelligence agent.
    ...
    sandbox   every retrieved byte through vulnpriority.sandbox, tier REFERENCE_PAGE
...
set. :func:`judge_as_of` returns an :class:`~vulnpriority.intel.models.AsOfVerdict` carrying a
...
Either way the provenance is stamped: every :class:`~vulnpriority.intel.models.IntelDocument`
...
from vulnpriority.core.models import Finding, LLMAudit, UntrustedText, VulnIntel
from vulnpriority.core.resolve import concrete_intel, resolve_backend, resolve_intel
...
from vulnpriority.llm.heuristic import sanitized_text_of, strip_imperative_sentences
from vulnpriority.sandbox.pipeline import Sandbox, build_sandboxed_prompt
from vulnpriority.semantic.criticality import apply_budget, clamp01, heuristic_audit
from vulnpriority.semantic.cpe_match import parse_version, version_in_range
from vulnpriority.semantic.exploitability import extract_maturity, fuse_maturity, structured_maturity
...
class IntelBaselineBackend(LLMBackend):
    ...
    The same role :class:`~vulnpriority.llm.heuristic.HeuristicBackend` plays for Component A,
...
class ExploitIntelAgent:
    ...
    def _fuse(
        ...
        :func:`~vulnpriority.semantic.exploitability.fuse_maturity` against the curated-feed
...
def fuse_into_exploitability(
    ...
    This lives here rather than in :mod:`vulnpriority.semantic.exploitability` so that the
