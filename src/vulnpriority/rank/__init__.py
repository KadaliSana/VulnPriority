...
:data:`vulnpriority.core.registry.RANKERS`, which is what makes
``get_ranker(RankerName.LAMBDAMART)`` and the benchmark runner's baseline sweep work. The
registration happens as a side effect of importing :mod:`vulnpriority.rank.lambdamart` and
:mod:`vulnpriority.rank.baselines`, so the names below are re-exported eagerly rather than
lazily.
...
from vulnpriority.rank.compose import order_within_groups, rank_scan
...
from vulnpriority.rank.lambdamart import FALLBACK_COLUMNS, LambdaMartRanker
from vulnpriority.rank.likelihood_head import CALIBRATION_METHODS, CostSensitiveExploitHead
