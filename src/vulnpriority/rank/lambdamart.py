...
from vulnpriority.core.models import Explanation, FeatureFrame
...
class LambdaMartRanker(Ranker):
    ...
    def explain(self, frame: FeatureFrame, top_n: int = 5) -> list[Explanation] | None:
        ...
        if self._model is None:
            return None
        from vulnpriority.rank.explain import ShapExplainer  # local import: avoids a cycle
    ...
    def load(cls, path: str | Path) -> "LambdaMartRanker":
        ...
        if not meta_path.exists():
            raise VulnPriorityError(f"no ranker metadata beside {target} (expected {meta_path})")
        ...
        if meta.get("has_booster", False):
            if not target.exists():
                raise VulnPriorityError(f"ranker metadata claims a booster but {target} is missing")
