...
:class:`~vulnpriority.sandbox.pipeline.Sandbox`: the sandbox stays the single source of truth for
what counts as a signal, and this class adds only what a *detector* needs and a *sanitizer*
does not - a bounded score and :class:`~vulnpriority.core.models.ManipulationAlert` emission.
...
from vulnpriority.core.interfaces import ManipulationDetector
...
from vulnpriority.sandbox.normalize import normalize_untrusted
from vulnpriority.sandbox.output_guard import span_matches
...
#: p_exploit floors implied by tier <= 1 evidence. These mirror ``vulnpriority.enrich.trust`` but
...
class PreLLMPatternDetector(ManipulationDetector):
    ...
    Delegates to :class:`~vulnpriority.sandbox.pipeline.Sandbox` so that what counts as a signal
    ...
    def detect(
        ...
        Implements :class:`~vulnpriority.core.interfaces.ManipulationDetector`, so the pipeline
