...
* **Gap 3 - circular validation.** :mod:`vulnpriority.eval.labels` accepts exploitation
...
* **Gap 4 - no standardised protocol.** :mod:`vulnpriority.eval.splits` gives time-ordered
  folds with a gap buffer (and a clearly labelled random control),
  :mod:`vulnpriority.eval.metrics` implements the fixed metric battery from first principles,
  :mod:`vulnpriority.eval.benchmark` runs every ranker on identical data, and
  :mod:`vulnpriority.eval.bootstrap` puts an interval on every comparison.
* **Gap 5 - missing component ablation.** :mod:`vulnpriority.eval.ablation` runs the full
  ``2^3`` factorial with main effects, interactions and paired intervals.
* **Gap 7 - class imbalance.** :mod:`vulnpriority.eval.minority` reports MCC, balanced
  accuracy and per-class F1 with support.
* **Gap 10 - evaluation stops at prediction.** :mod:`vulnpriority.eval.simulation` spends a
  weekly remediation capacity and measures exposure days.

:mod:`vulnpriority.eval.report` turns all of it into ``report.md``, ``report.json`` and
figures.
"""
...
from vulnpriority.eval.benchmark import BenchmarkRunner, SplitFrames, rank_from_scores, scan_order
...
from vulnpriority.eval.labels import (
    CVSS_DERIVED_TOKENS,
    LabelAudit,
    LabelBuilder,
...
from vulnpriority.eval.simulation import FindingState, LongitudinalSimulator, PolicyTrace
