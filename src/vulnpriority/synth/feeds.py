"""Synthetic feed fixtures in the exact shapes ``vulnpriority.feeds`` reads (DESIGN.md 3.11).
...
* ``nvd/cves.json`` in the NVD 2.0 envelope :class:`~vulnpriority.feeds.nvd.NvdFixtureFeed`
  ...
  :func:`~vulnpriority.feeds.cvss_policy.select_cvss` a real selection to make and produces a
  ``cvss_source_agreement`` below 1 (Gap 3);
* ``epss/epss_snapshots.jsonl`` as one row per (CVE, date), so the as-of rule in
  :class:`~vulnpriority.feeds.epss.EpssFixtureFeed` returns different evidence in March than in
  September;
...
* ``references/index.json`` for :class:`~vulnpriority.feeds.references.ReferenceFixtureFetcher`.
...
from vulnpriority.core.enums import ExploitMaturity
from vulnpriority.synth.world import LatentVuln, LatentWorld
...
def write_reference_index(pages: Sequence[dict[str, Any]], fixture_dir: str | Path) -> Path:
    """Write ``references/index.json`` from the pages built by :mod:`vulnpriority.synth.pages`."""
