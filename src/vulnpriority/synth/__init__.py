...
* :mod:`~vulnpriority.synth.topology` - application surfaces: hosts, routes, links, stacks.
* :mod:`~vulnpriority.synth.world` - the latent vulnerability universe and its noisy evidence.
* :mod:`~vulnpriority.synth.feeds` - fixtures in the exact shapes ``vulnpriority.feeds`` reads.
* :mod:`~vulnpriority.synth.pages` - multilingual reference pages, optionally injected.
* :mod:`~vulnpriority.synth.oracle` - exploitation events and remediation counterfactuals.
* :mod:`~vulnpriority.synth.generator` - :class:`SyntheticDataset`, which writes it all out.
"""

from __future__ import annotations

from vulnpriority.synth.feeds import FEED_FILES, write_feed_fixtures
from vulnpriority.synth.generator import (
    DEFAULT_SYNTHETIC_ROOT,
    LATENT_FUNCTION_VALUE,
    SyntheticDataset,
    synthetic_config_of,
)
from vulnpriority.synth.oracle import (
    HAZARD_WEIGHTS,
    ExploitationEvent,
    ExploitationOracle,
    LatentFinding,
...
from vulnpriority.synth.world import (
    VULN_TEMPLATES,
    EpssPoint,
    LatentVuln,
    LatentWorld,
