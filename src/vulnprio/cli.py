"""``vulnprio``: the command line application (DESIGN.md 3.11).
...
* A stage command runs everything it depends on, reloading whatever a previous invocation
  already completed. ``vulnprio rank`` after ``vulnprio enrich`` re-uses the enrichment
...
_CURRENCY_ERRORS = "vulnprio.currency"
...
def _run_through(
    ...
    *,
    ...
    Dependencies, not position: ``vulnprio select`` needs a ranking and labels, and must not
    ...
    runner = PipelineRunner(
        config=config,
        dataset=dataset,
        command=" ".join(["vulnprio"] + sys.argv[1:]),
...
def _echo_ranking_provenance(ranking: Any) -> None:
    ...
    if getattr(ranking, "model_fitted", False):
        ...
        typer.echo(
            ...
            "was available. The queue is ordered by expected loss. Run `vulnprio "
...
def synth(
    ...
    from vulnprio.synth.generator import DEFAULT_SYNTHETIC_ROOT, SyntheticDataset
    ...
    typer.echo(f"use with:      vulnprio run-all --dataset {dataset.root}")
...
def train_ranker(
    ...
    """
    from vulnprio.core.config import DEFAULT_RANKER_MODEL
    from vulnprio.pipeline.stages import train_ranker_stage
...
def explain(
    ...
    from vulnprio.pipeline.artifacts import RunArtifacts, find_run_root

    settings, _world = _with_dataset(_config(config, set_, output_dir), dataset)
    root = find_run_root(settings)
    if not root.exists():
        _fail(FileNotFoundError(f"no run directory at {root}; run `vulnprio rank` first"))
        return
...
def ablate(
    ...
    from vulnprio.pipeline.stages import ablate_stage
...
def _write_assessment_report(artifacts: Any, runner: Any) -> None:
    ...
    """
    try:
        from vulnprio.report import build_report
...
def run_all(
    ...
    if world is None and len(list(scan or ())) < settings.evaluation.min_train_scans:
        from vulnprio.pipeline.runner import STAGE_ORDER
        ...
        # intelligence to inject into. Run it deliberately with `vulnprio adversarial`.
        research_only = {
...
def manifest(
    ...
    from vulnprio.pipeline.artifacts import find_run_root, load_manifest
...
def web(
    ...
    from vulnprio.web.exporter import export_site
...
def scan_command(
    ...
    from vulnprio.scan import ScanProfile, ScanRequest, assess_target
    from vulnprio.scan.adapters import describe_external_tools
    from vulnprio.scan.safety import OutOfScopeError, normalise_target
    ...
    typer.echo(f"prioritise it with:  vulnprio run-all --input {destination}")
...
def novelty_command(
    ...
    from vulnprio.novelty import novelty_payload
...
def main() -> None:
    """Entry point for ``python -m vulnprio``.

    ``_harden_streams`` also runs from the Typer callback above, which is what covers the
    installed ``vulnprio`` console script: ``pyproject.toml`` points that at ``app``, not
