...
* **Per-scan retention.** Every metric is kept per test scan, not only as a fold mean,
  because the paired bootstrap and the Wilcoxon test in :mod:`vulnprio.eval.bootstrap`
  need the pairs. This is what lets the report say "better on 24 of 30 scans" rather
  than "0.71 versus 0.68".

``vulnprio.rank`` is imported **lazily, inside the resolution function**, so this module
...
from vulnprio.core.enums import MetricName, RankerName
...
from vulnprio.eval.bootstrap import bootstrap_mean_ci
from vulnprio.eval.calibration import reliability_bins
...
from vulnprio.eval.minority import minority_report
...
class BenchmarkRunner:
    ...
    def paired_values(
        ...
        This is the input :func:`vulnprio.eval.bootstrap.paired_bootstrap_ci` expects.
        """
...
def _resolve_ranker(item: Any, config: PipelineConfig) -> tuple[RankerName, Any]:
    ...
    ``vulnprio.rank`` is imported here and nowhere else in the module: importing it
    ...
    import vulnprio.rank  # noqa: F401  (registers the built-in rankers)
