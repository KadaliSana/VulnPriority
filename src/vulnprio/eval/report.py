...
from vulnprio.core.money import DEFAULT_CURRENCY, format_money
...
from vulnprio.eval.ablation import COMPONENT_KEYS, INTERACTION_KEYS
from vulnprio.eval.simulation import prevention_rate, summarise
...
class ReportBuilder:
    ...
    def __init__(
        self,
        title: str = "vulnprio evaluation report",
...
def _capacity_lines(simulations: Sequence[SimulationResult], capacity: Any | None) -> list[str]:
    ...
    ``capacity`` is a :class:`~vulnprio.eval.simulation.CapacityContext` when the caller
