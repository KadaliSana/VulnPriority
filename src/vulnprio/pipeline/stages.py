...
:mod:`vulnprio.pipeline.runner`'s job. Keeping the two apart is what lets the CLI run a
...
from vulnprio.core.config import (
    PipelineConfig,
    load_attacker_preset,
    load_impact_preset,
)
from vulnprio.core.enums import RankerName, SelectionMethod
from vulnprio.core.errors import ConfigError, VulnprioError
...
from vulnprio.enrich.enricher import ContextualEnricher
from vulnprio.feeds.bundle import DefaultIntelAssembler, build_feed_bundle
from vulnprio.ingest.correlate import FindingCorrelator
from vulnprio.ingest.generic import parse_scan
from vulnprio.llm.factory import build_llm_backend
from vulnprio.sandbox.pipeline import Sandbox
from vulnprio.semantic.assessor import AgenticSemanticAssessor
...
class StageUnavailableError(VulnprioError):
...
def chain_stage(
    ...
    scorer_class = _import("vulnprio.graph.chain_scorer", "ReachabilityChainScorer")
...
def label_stage(
    ...
    *,
    ...
    ``oracle`` is the synthetic :class:`~vulnprio.synth.oracle.ExploitationOracle` when the
    ...
    """
    builder_class = _import("vulnprio.eval.labels", "LabelBuilder")
...
def feature_stage(
    ...
    *,
    ...
    """
    builder_class = _import("vulnprio.rank.features", "FeatureBuilder")
...
def build_ranker(config: PipelineConfig, name: RankerName | str | None = None, seed: int | None = None) -> Any:
    """Construct the named ranker from the registry that importing ``vulnprio.rank`` fills.

    Each ranker takes what it actually needs - the learned model takes
    :class:`~vulnprio.core.config.RankingConfig`, the random control takes a seed, the VMC
    ...
    """
    _import("vulnprio.rank")  # registers the built-ins
    get_ranker = _import("vulnprio.core.registry", "get_ranker")
...
def rank_stage(
    ...
    *,
    ...
    """
    rank_scan = _import("vulnprio.rank.compose", "rank_scan")
    cell = flags or config.flags()
...
def _explainer_for(config: PipelineConfig, model: Any) -> Any | None:
    ...
    try:
        explainer_class, lambdamart = _import("vulnprio.rank.explain", "ShapExplainer"), _import(
            "vulnprio.rank.lambdamart", "LambdaMartRanker"
        )
...
def _manipulation_alerts(
    ...
    try:
        detector_class = _import("vulnprio.rank.rank_guard", "RankManipulationDetector")
    except StageUnavailableError:  # pragma: no cover - depends on the install
        return []
...
def split_stage(
    ...
    *,
    ...
    build_splitter = _import("vulnprio.eval.splits", "build_splitter")
...
def evaluate_stage(
    ...
    *,
    ...
    """
    runner_class = _import("vulnprio.eval.benchmark", "BenchmarkRunner")
    cell = flags or config.flags()
...
def ablate_stage(
    ...
    *,
    ...
    """
    ablation_class = _import("vulnprio.eval.ablation", "FullFactorialAblation")
    chain = _flatten_chain(chain_scores)
    folds = list(splits) if splits is not None else split_stage(config, scans, labels)
...
def select_stage(
    ...
    **It is per policy because the comparison is the result.** "57% of risk captured in 38
    ...
    select_under_budget, item_class = _import(
        "vulnprio.select.knapsack", "select_under_budget", "SelectionItem"
    )
    method_enum = _import("vulnprio.core.enums", "SelectionMethod")
...
def build_simulator() -> Any:
    ...
    return _import("vulnprio.eval.simulation", "LongitudinalSimulator")()
...
def capacity_from_payload(payload: Any | None) -> Any | None:
    ...
    if payload is None or not isinstance(payload, Mapping):
        return payload
    try:
        context_class = _import("vulnprio.eval.simulation", "CapacityContext")
...
def simulate_stage(
    ...
    *,
    ...
    first-evidence date onto each :class:`~vulnprio.core.models.GroundTruthLabel`, and the
...
def adversarial_stage(
    ...
    *,
    ...
    """
    evaluator_class = _import("vulnprio.adversarial.evaluator", "AdversarialEvaluator")
    load_corpus = _import("vulnprio.adversarial.corpus", "load_corpus")
...
def report_stage(
    ...
    *,
    ...
    """
    builder_class = _import("vulnprio.eval.report", "ReportBuilder")
