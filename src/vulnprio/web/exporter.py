...
framework's whole claim is that it runs offline and reproducibly. ``vulnprio web --serve``
...
from vulnprio.core.enums import Component, RankerName, TrustTier
from vulnprio.core.money import DEFAULT_CURRENCY, format_money, format_money_compact
...
def _guidance_lookup() -> Any:
    """``vulnprio.report.guidance_for`` when that package is installed, else ``None``.
    ...
    """
    try:
        from vulnprio.report import guidance_for  # type: ignore[attr-defined]
...
def write_payload(data: DashboardData, out_dir: Path) -> tuple[Path, Path]:
    ...
    js_path.write_text(f"window.VULNPRIO_DATA = {text};\n", encoding="utf-8")
