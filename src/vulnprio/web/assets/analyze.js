/* vulnprio - the interactive half of the page.
 *
 ...
 * is never fetched. Everything here talks to vulnprio.web.app over fetch; nothing here
...
(() => {
  ...
  function tokenHeader() {
    const name = SESSION.token_header || "X-Vulnprio-Token";
  ...
  async function renderScanners() {
    ...
    if (usable.length) {
      ...
      html += option("", `Best available: ${best}`, "vulnprio selects the strongest installed scanner.", true);
      usable.forEach((tool) => {
        html += option(
          tool.name,
    ...
    html += option(
      ...
      usable.length
        ? "vulnprio's own crawler. Conservative by design; it finds less than a dedicated scanner."
        : "vulnprio's own crawler. Conservative by design; it finds less than a dedicated scanner.",
      !usable.length);
  ...
  function fillPresets() {
    ...
    if (!HEALTH.capabilities || !HEALTH.capabilities.scan) {
      ...
      if (button) {
        ...
        button.title = "The vulnprio.scan package is not installed in this build.";
      }
    }
  }
  ...
  async function detectScanner(file) {
    ...
    if (lower.includes('"scan_id"') && lower.includes('"findings"')) return "vulnprio scan";
  ...
  async function inspectChosenFile(file) {
    ...
    try {
      ...
      console.warn("vulnprio: could not inspect the report - ", error.message);
    }
  }
   ...
   *  server normalises identically through vulnprio.scan.safety.normalise_target; this
  ...
  function renderReport() {
    ...
    empty.hidden = true;
    const intel = ((window.VULNPRIO_DATA || {}).notes || {}).intel || {};
  ...
  function wireReportDownloads() {
    $("reportControls").addEventListener("click", (event) => {
      ...
      anchor.download = `vulnprio-report.${button.dataset.fmt}`;
    ...
    if (data.available === false) {
      $("noveltyBanner").innerHTML =
        `<div class="banner"><b>Not available in this build.</b> ${esc(data.detail ||
          "The vulnprio.novelty package is not installed, so there is nothing to show. " +
