...
The static export in :mod:`vulnprio.web.exporter` answers "what did that run find?". This
...
back the same :class:`~vulnprio.web.schema.DashboardData` payload the static site consumes.
...
The pipeline runs on a worker thread owned by :mod:`vulnprio.web.jobs`, never on the event
...
* a per-process random token, minted at startup and written into the page it serves, is
  ...
  start a scan on the operator's behalf;
...
* an authorisation flag *and* a written note are required before a scan request is even
  ...
  reaches :mod:`vulnprio.scan`;
* no stack trace is ever written to a response; failures are logged server-side and the
  client is told what failed, not where.

Run it with ``python -m vulnprio.web.app`` (add ``--demo`` to preload the worked example),
or point any ASGI server at ``vulnprio.web.app:app``. Interactive API documentation is at
``/docs``.
"""
...
from vulnprio.core.money import format_money, format_money_compact
from vulnprio.web.jobs import Job, JobContext, JobStatus, JobStore
...
LOGGER = logging.getLogger("vulnprio.web.app")
...
SESSION_MARKER = "window.VULNPRIO_SESSION = null;"

TOKEN_HEADER = "X-Vulnprio-Token"
...
#: :mod:`vulnprio.scan` is the authority; these are used when it does not describe itself,
...
FALLBACK_PROFILES: tuple[dict[str, Any], ...] = (
    {
        ...
        "sends": [
            ...
            "An identifying User-Agent naming vulnprio",
...
def probe_capabilities(refresh: bool = False) -> WebCapabilities:
    ...
    with _CAPABILITY_LOCK:
        ...
        for key, module, attribute in (
            ("scan", "vulnprio.scan", "assess_target"),
            ("report", "vulnprio.report", "build_report"),
            ("novelty", "vulnprio.novelty", "novelty_payload"),
        ):
            try:
...
def _scan_profiles(capabilities: WebCapabilities) -> list[WebScanProfile]:
    """What each profile actually does, read out of :mod:`vulnprio.scan` where possible.
    ...
    try:
        scan = importlib.import_module("vulnprio.scan")
        ...
        agent = str(getattr(scan, "DEFAULT_USER_AGENT", "")).split(" ")[0] or "vulnprio-scan"
        probes = sorted(
...
def scanner_environment(profile: str = "passive") -> Any:
    ...
    them apart. :mod:`vulnprio.scan` already works all this out, including the awkward
    ...
    """
    from vulnprio.web.schema import WebScannerEnvironment, WebScannerTool

    try:
        import vulnprio.scan as scan
    except Exception as error:  # noqa: BLE001
        return WebScannerEnvironment(
            profile=profile,
            available=False,
            notice=f"This build cannot assess a target: vulnprio.scan did not import ({error}).",
        )
    try:
...
def judge_report_freshness(scanned_at: datetime | None, *, live: bool) -> Any:
    ...
    Delegated to :func:`vulnprio.intel.agent.judge_as_of`, which owns the rule and the
    ...
    """
    from vulnprio.web.schema import WebAsOf
    ...
    try:
        from vulnprio.intel.agent import judge_as_of
        from vulnprio.intel.models import IntelConfig
    except Exception as error:  # noqa: BLE001 - no live search in this build
        LOGGER.debug("vulnprio.intel unavailable for an as-of judgement: %s", error)
        return WebAsOf(
            status="not_applicable",
            as_of=scan_date,
...
def inspect_report(path: Path, *, live_intel: bool) -> Any:
    ...
    from vulnprio.web.schema import WebReportInspection
...
def _scan_progress(context: JobContext, low: float, high: float) -> Callable[..., None]:
    """An ``on_progress`` for :func:`vulnprio.scan.assess_target`.
...
def _scan_target(request: AnalyzeRequest, context: JobContext) -> tuple[Any, dict[str, Any]]:
    """Run :mod:`vulnprio.scan` against the authorised target.
    ...
    """
    try:
        module = importlib.import_module("vulnprio.scan")
        ...
        raise AnalysisUnavailable(
            "Assessing a target needs the vulnprio.scan package, which this build cannot "
    ...
    try:
        from vulnprio.scan.safety import normalise_target
    ...
    try:
        ...
        raise AnalysisUnavailable(
            f"vulnprio.scan rejected the request this page built ({type(error).__name__}: "
    ...
    if scan is None:
        raise AnalysisUnavailable(
            "vulnprio.scan returned an outcome with no .scan on it, so there is nothing to "
...
def run_analysis(
    ...
    *,
    ...
    :func:`vulnprio.web.exporter.build_dashboard`. Evaluation and ablation need labelled
...
def _run_analysis(
    ...
    *,
    ...
    from vulnprio.pipeline import stages
    ...
    try:
        if request.mode == "upload":
            path = source_path
            if path is None:
                workdir = tempfile.TemporaryDirectory(prefix="vulnprio-upload-")
...
def _manifest_for(config: Any, scans: Sequence[Any], run_id: str) -> Any:
    ...
    try:
        from vulnprio.pipeline.runner import build_manifest

        return build_manifest(
            config, f"web-{run_id[:12]}", scans=list(scans),
            command="vulnprio.web.app /api/analyze",
        )
...
def build_report_document(
    ...
    :mod:`vulnprio.report` owns this when it is present. When it is not - or when it
    ...
    try:
        build_report = importlib.import_module("vulnprio.report").build_report
    except Exception as error:  # noqa: BLE001
        LOGGER.info("vulnprio.report unavailable (%s); writing the built-in summary", error)
        return _fallback_report(
            data, fmt, reason=f"vulnprio.report is not available in this build ({error})."
        )

    # ``theme`` and ``embedded`` are presentation-only and only the HTML renderer knows what
    # to do with them. An older vulnprio.report will not accept them, so they are offered
    ...
    try:
        ...
        LOGGER.exception("vulnprio.report failed for format %s", fmt)
        return _fallback_report(
            data, fmt, reason=f"vulnprio.report raised {type(error).__name__}: {error}."
        )
...
def _fallback_report_markdown(data: DashboardData, reason: str) -> str:
    ...
    add("# vulnprio - prioritised findings")
    add("")
    add(
        f"Generated {datetime.now(timezone.utc).isoformat(timespec='seconds')} by vulnprio "
...
<h1>vulnprio - prioritised findings</h1>
<div>Generated {esc(datetime.now(timezone.utc).isoformat(timespec="seconds"))}
 by vulnprio {esc(meta.package_version or "")}.</div>
...
class WebApplication:
    ...
    def index_html(self) -> str:
        ...
        injected = f"window.VULNPRIO_SESSION = {session};"
        ...
        if not capabilities.novelty:
            return {
                ...
                "detail": capabilities.detail.get(
                    "novelty", "vulnprio.novelty is not available in this build."
                ),
            }
        try:
            produced = importlib.import_module("vulnprio.novelty").novelty_payload()
        except Exception as error:  # noqa: BLE001
            LOGGER.exception("novelty_payload() failed")
            return {
                "available": False,
                "detail": f"vulnprio.novelty.novelty_payload() failed: "
...
def _state(request: Request) -> WebApplication:
    return request.app.state.vulnprio  # type: ignore[no-any-return]


def require_token(
    request: Request,
    supplied_header: str | None = Header(None, alias=TOKEN_HEADER),
    token: str | None = Query(None, description="Session token, if a header is inconvenient."),
) -> None:
    """Mutating endpoints need the token this process minted at startup.

    It is not authentication - anything that can read the served page has it. It stops a
    page in another browser tab from silently posting a scan request to localhost, which is
    the one attack this threat model actually has to answer.
    """
    application = _state(request)
    candidate = supplied_header or token
...
def _register_routes(api: FastAPI) -> None:
    ...
    async def data_js(request: Request) -> Response:
        ...
        return Response(
            content=f"window.VULNPRIO_DATA = {text};\n",
 ...
 - so this is queried per profile rather than answered once.
    ...
    async def inspect_upload(
        ...
        application = _state(request)
        name = _safe_report_name(file.filename)
        workdir = tempfile.TemporaryDirectory(prefix="vulnprio-inspect-")
        try:
            path = Path(workdir.name) / name
            await _stream_upload(file, path, application.max_body_bytes)
    ...
    async def job_report(
        ...
        headers = (
            {"Content-Disposition": f'attachment; filename="vulnprio-report.{fmt.lower()}"'}
            if download else {}
        )
...
def create_app(config: AppSettings | None = None, **kwargs: Any) -> FastAPI:
    ...
    api.state.vulnprio = application
...
#: The default application, for ``uvicorn vulnprio.web.app:app``.
app = create_app()
...
def serve(
    ...
    *,
    ...
    api = api or create_app(AppSettings(data=data, allow_scan=allow_scan))
    if host not in ("127.0.0.1", "localhost", "::1"):
        LOGGER.warning(
            "vulnprio's interactive server is bound to %s, not loopback. It has no "
...
def main(argv: list[str] | None = None) -> int:
    """``python -m vulnprio.web.app`` - start the interactive site on localhost."""
    parser = argparse.ArgumentParser(
        prog="python -m vulnprio.web.app",
        description="Serve the interactive vulnprio site (local tool; do not expose it).",
    )
    ...
    api = create_app(AppSettings(data=data, allow_scan=not args.no_scan))
    capabilities = probe_capabilities()
    print(f"vulnprio interactive site at http://{args.host}:{args.port}/   (docs at /docs)")
