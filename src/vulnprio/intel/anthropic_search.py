...
to produce a bounded :class:`~vulnprio.intel.models.ExploitIntelOut`.
...
from vulnprio.intel.queries import PHASE1_SYSTEM
from vulnprio.sandbox.delimit import envelope
...
def build_client(config: IntelConfig, api_key: str) -> Any:
    ...
    try:
        ...
        raise ConfigError(
            ...
            "install vulnprio[llm] or use mode='offline'"
        ) from exc
...
class AnthropicIntelExtractor(LLMBackend):
    ...
    This is an ordinary :class:`~vulnprio.core.interfaces.LLMBackend`, which is the point:
