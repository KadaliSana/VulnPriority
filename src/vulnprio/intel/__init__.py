...
with a schema and no tools. :mod:`vulnprio.intel.anthropic_search` explains the request
shapes; :mod:`vulnprio.intel.agent` explains the as-of discipline, which is the subtlest
thing here.

:mod:`vulnprio.intel.parallel_search` is the third way to do phase one and the only one
...
* **Untrusted by default.** Every retrieved byte is
  :class:`~vulnprio.core.models.UntrustedText` at tier ``REFERENCE_PAGE`` and reaches a
  model only through :mod:`vulnprio.sandbox.pipeline`. Phase two runs under the existing
...
* **As-of dated.** Every document is stamped with ``retrieved_at``, and
  :func:`~vulnprio.intel.agent.judge_as_of` decides whether searching now can honestly
...
:data:`~vulnprio.intel.models.INTEL_FEATURE_NAMES` -- that the learned ranker consumes
...
from vulnprio.intel.gemini_search import GeminiSearchProvider, grounding_documents
...
from vulnprio.intel.offline import FixtureSearchProvider, RecordingProvider
