...
**Cost.** :mod:`vulnprio.intel.anthropic_search` works and bills per finding.
:mod:`vulnprio.intel.gemini_search` is free in principle, but grounding carries its own
...
therefore no report summary.** :func:`~vulnprio.intel.summarize.summarize_intel` returns
...
:attr:`~vulnprio.core.config.IntelConfig.parallel_mode` defaults to ``turbo`` and reaches
the other three.
...
* ``turbo`` returns in about a second -- 1578ms cold, then 1051ms and 1028ms -- not the
  ~200ms advertised. :attr:`~vulnprio.core.config.IntelConfig.parallel_timeout_s` is 30s,
...
* Excerpts are raw scraped page text, newlines and navigation furniture included ("You must
  ...
  :mod:`vulnprio.sandbox.pipeline`'s job, and a second, unaudited cleaner would be a second
...
where to look, which is precisely how :mod:`vulnprio.intel.queries` already decomposes a
finding.
...
set and which :class:`~vulnprio.core.models.IntelDocument` enforces. A search API surfacing
...
from vulnprio.core.enums import IntelQueryKind
...
from vulnprio.intel.gemini_search import host_allowed
from vulnprio.llm.gemini_backend import is_rate_limited
from vulnprio.llm.openai_compatible import RateLimiter, backoff_delay, retry_after_seconds
...
#: :class:`~vulnprio.core.config.IntelConfig` as defaults so both are configurable, and kept
...
#: :meth:`~vulnprio.core.models.IntelUsage.cost_usd` errs in for cache reads.
...
#: :mod:`vulnprio.intel.offline` corpus shape, for the reason that module's docstring gives
...
def build_objective(queries: Sequence[IntelQuery], instruction: str = "") -> str:
    ...
    :mod:`vulnprio.intel.queries` states: an objective built from a page would let a page
...
def search_body(
    ...
    :attr:`~vulnprio.core.config.IntelConfig.max_queries`, the same ceiling the search plan
...
def excerpt_text(excerpts: Any, max_chars: int) -> str:
    ...
    **The truncation is ours, not the API's** -- Parallel documents no length parameter, and
    ...
    all, and normalising it is :mod:`vulnprio.sandbox.pipeline`'s job. Cleaning it here
...
def _rank_relevance(index: int) -> float:
    ...
    :func:`~vulnprio.intel.queries.dedupe_documents` and the influence budget see a
...
def parallel_documents(
    ...
    2. :attr:`~vulnprio.core.config.IntelConfig.parallel_max_excerpt_chars` per excerpt;
    3. :attr:`~vulnprio.core.config.IntelConfig.max_documents` per finding, enforced by
    ...
    :class:`~vulnprio.core.models.IntelDocument` has no publication field and it is not this
...
def rate_limited(status_code: int, exc: BaseException | None = None) -> bool:
    ...
    :func:`~vulnprio.llm.gemini_backend.is_rate_limited` rather than re-derived, so the
...
def load_recorded_responses(path: str | Path | None = None) -> dict[str, Any]:
    ...
    Raw responses rather than the :mod:`vulnprio.intel.offline` corpus shape: these exist so
    ...
    offline corpus is :class:`~vulnprio.intel.offline.RecordingProvider`'s job, which works
...
class ParallelSearchProvider(BaseSearchProvider):
    ...
    def parse(
        ...
        page, and :class:`~vulnprio.core.models.IntelCitation` means "a span a model
