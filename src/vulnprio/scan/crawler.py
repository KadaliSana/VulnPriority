...
* **Scope.** A link off the authorised host is dropped and counted before it is queued, so
  the out-of-scope URL is never requested at all (:mod:`vulnprio.scan.safety` re-checks it
...
* **Deduplication by templated path.** Visits are keyed on method, host, the
  :func:`~vulnprio.ingest.normalize.template_path` form of the path and the sorted query
...
:mod:`vulnprio.scan.spa` mines the request paths the client code names in string
...
from vulnprio.scan.http import HttpClient
...
from vulnprio.scan.safety import OutOfScopeError, RobotsPolicy, host_in_scope, is_http_url
