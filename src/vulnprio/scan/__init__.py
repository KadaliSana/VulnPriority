...
Everything else in vulnprio starts from a scanner report. This package removes that
...
:class:`vulnprio.core.models.Scan` identical in shape to a parsed OWASP ZAP report - same
...
**A real scanner runs when one is installed.** :func:`~vulnprio.scan.runner.assess_target`
...
afterwards. :func:`~vulnprio.scan.adapters.scanner_environment` reports what is installed,
what each tool supports and how to install what is missing.

The safety model, in full:

* **Authorisation is mandatory.** :class:`~vulnprio.scan.models.ScanRequest` carries
  ``authorized`` (default ``False``) and ``authorization_note``;
  :func:`~vulnprio.scan.safety.require_authorization` raises
  :class:`~vulnprio.scan.safety.NotAuthorizedError` otherwise. There is no bypass.
...
* **Active means three benign probes.** A random alphanumeric reflection marker, an
  ...
  (:data:`~vulnprio.scan.safety.ALLOWED_PROBE_KINDS`) and enforced in code.
...
Typical use::
    ...
    from vulnprio.scan import ScanRequest, assess_target
...
from vulnprio.scan.checks import CHECKS, Check, CheckContext, checks_for_profile, run_checks
from vulnprio.scan.crawler import Crawler, extract_forms, extract_links, visit_key
from vulnprio.scan.http import FetchResult, HttpClient, fetch_robots_policy
...
from vulnprio.scan.passive import PASSIVE_CHECKS, run_passive_checks
from vulnprio.scan.runner import assess_target, build_scan, run_scan
