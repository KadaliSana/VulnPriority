...
:data:`vulnprio.scan.safety.ALLOWED_PROBE_KINDS` and enforced by
:func:`~vulnprio.scan.safety.require_allowed_probe`, which every probe in this module
...
1. **Reflected marker.** A random alphanumeric string is sent as the value of one existing
   ...
   :func:`~vulnprio.scan.safety.assert_benign_marker` to contain only letters and digits,
...
These are not conventions. :data:`vulnprio.scan.safety.ALLOWED_PROBE_KINDS` is a closed
