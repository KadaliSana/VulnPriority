...
The product of an assessment is a :class:`vulnprio.core.models.Scan` built with the same
helpers a parsed ZAP report goes through - :class:`~vulnprio.ingest.normalize.EndpointAccumulator`
for endpoints and auth inference, :func:`~vulnprio.ingest.normalize.make_finding_id` and
friends for identifiers, :func:`~vulnprio.ingest.tech_fingerprint.fingerprint_response`
for the technology stack, and :class:`~vulnprio.ingest.correlate.FindingCorrelator` for
...
sets :attr:`~vulnprio.scan.models.ScanRequest.scanned_at` produces identical identifiers
...
from vulnprio.core.models import Finding, Scan, TechComponent
...
from vulnprio.ingest.tech_fingerprint import fingerprint_library, fingerprint_response, merge_tech
from vulnprio.scan.active import run_active_probes
...
from vulnprio.scan.checks import CheckContext, run_checks
from vulnprio.scan.crawler import Crawler
from vulnprio.scan.http import HttpClient, fetch_robots_policy
...
from vulnprio.scan.safety import Budget, RateLimiter, RobotsPolicy, require_authorization, require_target_allowed
...
def default_cve_lookup(as_of: date | None = None) -> Callable[[str, date | None], bool] | None:
    ...
    """
    try:
        from vulnprio.feeds.nvd import NvdFixtureFeed
...
def build_scan(
    ...
    *,
    ...
    Every identifier comes from :mod:`vulnprio.ingest.normalize`, so an endpoint this
    ...
    *,
    ...
    try:
        ...
        return ScanOutcome(
            ...
            tool_selection=(
                "the built-in vulnprio scanner produced these findings; it is deliberately "
...
def assess_target(
    ...
    **A real scanner runs when one is installed.** ZAP and Nuclei are better web scanners
    ...
    The tools are ranked (see :data:`~vulnprio.scan.adapters.TOOL_PREFERENCE`, overridable
    ...
    if force_builtin or not prefer_external:
        reason = (
            "the built-in vulnprio scanner was requested explicitly"
...
def _fallback_reason(
    ...
    from vulnprio.scan.adapters import install_hint_for

    if not available:
        return (
            "No external scanner was found on PATH, so the built-in vulnprio scanner ran. "
            "It is deliberately conservative and finds less than a dedicated scanner. "
            + install_hint_for(request.profile)
        )
    if attempts:
        return (
            "The built-in vulnprio scanner ran because no installed tool could serve this "
            f"{request.profile.value} scan: " + "; ".join(attempts)
        )
    return (  # pragma: no cover - unreachable: tools exist and none was skipped or tried
        "The built-in vulnprio scanner ran."
    )
