...
from vulnprio.core.interfaces import ProbabilityModel
from vulnprio.core.models import FeatureFrame
...
class CostSensitiveExploitHead(ProbabilityModel):
    ...
    def _as_binary(y: np.ndarray, rows: int) -> np.ndarray:
        ...
        if values.shape[0] != rows:
            raise VulnprioError(f"y has {values.shape[0]} entries for {rows} rows")
