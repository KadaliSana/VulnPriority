...
exception is ``a_intel_documents``, which ``vulnprio.intel`` specifies as ``log1p`` because
...
from vulnprio.attacker.likelihood import function_ordinal
...
from vulnprio.feeds.cvss_policy import cvss_features
from vulnprio.ingest.normalize import method_is_state_changing

if TYPE_CHECKING:  # pragma: no cover - typing only
    # Imported for annotations alone. ``vulnprio.intel`` reaches the ranking through the
...
#: The Component A features contributed by ``vulnprio.intel``, in emission order. Mirrors
#: ``vulnprio.intel.models.INTEL_FEATURE_NAMES``; a test asserts the two agree, so the two
...
if _MISSING_NEUTRALS or _MISSING_DOCS:  # pragma: no cover - import-time contract check
    raise ImportError(
        "vulnprio.rank.features is out of step with FEATURE_SPECS: "
...
if _EXTRA:  # pragma: no cover - import-time contract check
    raise ImportError(f"vulnprio.rank.features documents unknown features: {_EXTRA}")
...
class FeatureBuilder:
    ...
    def _intel_features(intel: "IntelResult | None") -> dict[str, float]:
        ...
        The derivation is deliberately thin: ``vulnprio.intel`` owns the extraction and
        ...
        if missing:  # pragma: no cover - contract check against a package in flux
            raise VulnprioError(
                ...
                "vulnprio.intel and vulnprio.rank.features disagree about INTEL_FEATURES"
            )
        return {
            name: (
