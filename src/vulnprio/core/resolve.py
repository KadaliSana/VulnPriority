...
fixture run. The pattern is :class:`~vulnprio.core.models.AsOfVerdict`'s: a machine-readable
...
from vulnprio.core.enums import FeedMode, LLMBackendKind

if TYPE_CHECKING:  # pragma: no cover - typing only, avoids an import cycle
    from vulnprio.core.config import FeedsConfig, IntelConfig, LLMConfig, PipelineConfig
    ...
    for kind, requirement in BACKEND_PROBE_ORDER:
        if kind is LLMBackendKind.OPENAI_COMPATIBLE:
            ...
            if not llm.model or llm.model == default_model:
                ...
                continue
            from vulnprio.llm.openai_compatible import (  # noqa: PLC0415 - local
                DEFAULT_KEY_ENV,
                is_local_endpoint,
            )
