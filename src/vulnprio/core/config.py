...
from vulnprio.core.models import AttackerModel, ComponentFlags, ImpactModel, LabelPolicy
from vulnprio.core.money import DEFAULT_CURRENCY

if TYPE_CHECKING:  # pragma: no cover - typing only, avoids an import cycle
    from vulnprio.core.resolve import RunResolution
...
#: Where ``vulnprio train-ranker`` writes, and where a run looks for a model to score with.
...
class FeedsConfig(BaseModel):
    ...
    #: is unreachable; see :mod:`vulnprio.core.resolve`. Writing ``offline`` explicitly
...
class LLMConfig(BaseModel):
    ...
    #: settles on ``heuristic`` when none is; see :data:`vulnprio.core.resolve
...
class RankingConfig(BaseModel):
    ...
    #:
    #: ``vulnprio train-ranker`` writes one. The default points at the model that ships with
...
class IntelConfig(BaseModel):
    """Internet exploit-intelligence retrieval (produced by :mod:`vulnprio.intel`).
    ...
    protocol -- see :func:`vulnprio.core.resolve.resolve_intel`. What has not changed is
...
class PipelineConfig(BaseModel):
    ...
    def resolution(self, probe: bool = True) -> "RunResolution":
        ...
        """
        from vulnprio.core.resolve import resolve_run  # noqa: PLC0415 - avoids a cycle
