...
``vulnprio.eval.report`` produces the *research* report - metrics, baselines, ablation,
...
Its input is the dashboard payload (:class:`~vulnprio.web.schema.DashboardData`), so the
...
from vulnprio.report.model import (
    RESEARCH_SECTIONS,
    AssessmentReport,
    ReportMeta,
    ReportOptions,
...
from vulnprio.report.remediation import (
    GENERIC_REMEDIATION,
    REMEDIATIONS,
    Remediation,
    guidance_for,
...
from vulnprio.report.render_html import render_html
from vulnprio.report.render_md import render_markdown
...
def build_report(
    ...
    *,
    ...
    ----------
    data:
        A :class:`~vulnprio.web.schema.DashboardData` or its dictionary form.
    fmt:
        ``"md"`` for GitHub-flavoured Markdown, ``"html"`` for one self-contained document,
        ``"json"`` for the :class:`~vulnprio.report.model.AssessmentReport` dumped, so the
    ...
    options:
        ...
        :class:`~vulnprio.report.model.ReportOptions`.
    theme:
