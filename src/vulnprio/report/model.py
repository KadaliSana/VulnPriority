...
It is not the research report in ``vulnprio/eval/report.py``, which argues that the method
...
* **Nothing is empty.** A section with no data is omitted and listed in
  ...
  apology under it.
* **Nothing is generated.** All prose comes from :mod:`vulnprio.report.narrative`, which is
...
from vulnprio.report import narrative as nv
from vulnprio.report.remediation import Remediation, guidance_for, known_cwes, weakness_name
...
def build_report_model(
    ...
    *,
    ...
    ``data`` may be a :class:`~vulnprio.web.schema.DashboardData` or the dictionary form of
...
def _section_header(
    ...
    rows = [
        ...
        ["Framework version", f"vulnprio {meta.framework_version}"],
        ["Run id", meta.run_id],
