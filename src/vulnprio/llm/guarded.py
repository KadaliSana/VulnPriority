...
from vulnprio.core.errors import (
    CanaryLeakError,
    EvidenceSpanError,
...
from vulnprio.llm.consistency import ConsistencyGuard
from vulnprio.llm.heuristic import HeuristicBackend, sanitized_text_of
from vulnprio.llm.schemas import BoundedOut
...
class OutputGuardLike(Protocol):
    """Structural type of ``vulnprio.sandbox.output_guard.OutputGuard``.

    Only ``check`` is required. It should raise
    :class:`~vulnprio.core.errors.SandboxViolationError` (or a subclass) when the answer
    must be rejected.
    """
...
def default_output_guard() -> Any | None:
    ...
    try:  # pragma: no cover - depends on sibling module availability
        from vulnprio.sandbox.output_guard import OutputGuard  # noqa: PLC0415
