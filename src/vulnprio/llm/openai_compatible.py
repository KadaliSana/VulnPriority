...
seven providers above. Missing either is a :class:`~vulnprio.core.errors.ConfigError` at
...
provider is asked to produce JSON (see :class:`~vulnprio.core.config.LLMConfig`), and the
...
:mod:`vulnprio.llm.gemini_backend` -- :class:`RateLimiter` and the 429 backoff -- because
...
def openai_json_schema(schema: type[BoundedOut]) -> dict[str, Any]:
    ...
    ``extra="forbid"`` on :class:`~vulnprio.llm.schemas.BoundedOut` already puts
