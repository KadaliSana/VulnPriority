...
:class:`~vulnprio.llm.schemas.ExploitabilityOut`: ``types.Schema.enum`` is typed
...
paces itself client-side (:class:`~vulnprio.llm.openai_compatible.RateLimiter`), treats a
...
from vulnprio.llm.openai_compatible import RateLimited, RateLimiter, backoff_delay
...
#: the one that carries free Google Search grounding for :mod:`vulnprio.intel`.
...
class GeminiBackend(LLMBackend):
    ...
    def __init__(
        ...
        try:
            ...
            raise ConfigError(
                "the 'google-genai' package is not installed; install vulnprio[gemini]"
            ) from exc
        self._types = _import_types()
