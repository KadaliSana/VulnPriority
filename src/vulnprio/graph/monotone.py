...
:mod:`vulnprio.graph.reachability`: maximum-probability paths computed as shortest paths
...
A failure is a :class:`vulnprio.core.errors.MonotonicityViolationError`, never a warning
...
def _finding_ids(scorer: ChainScorer, scan_id: str) -> tuple[str, ...]:
    ...
    (as :class:`~vulnprio.graph.chain_scorer.ReachabilityChainScorer` does) is asked
